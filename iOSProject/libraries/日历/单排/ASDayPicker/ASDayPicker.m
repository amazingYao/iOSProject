...
- (void)setup {
    ...
    self.beginDate = [self dateWithoutTimeFromDate:[NSDate date]];
...
- (void)recenter {
    ...
    if (_startDate) {
        ...
        weeks.week = -1;
        NSDate *prevDate = [_calendar dateByAddingComponents:weeks toDate:self.beginDate options:0];
    ...
    if (_endDate) {
        NSDateComponents *weeks = [[NSDateComponents alloc] init];
        weeks.week = 1;
        NSDate *nextDate = [_calendar dateByAddingComponents:weeks toDate:self.beginDate options:0];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (dir != 0) {
        NSDateComponents *weeks = [[NSDateComponents alloc] init];
        weeks.week = dir;
        NSDate *date = [_calendar dateByAddingComponents:weeks toDate:self.beginDate options:0];
...
- (NSArray*)daysForWeekAtIndex:(NSInteger)i {
    ...
    NSDate *d = [_calendar dateByAddingComponents:weeks toDate:self.beginDate options:0];
